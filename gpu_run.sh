...
python main.py --config=rl4im --env-config=basic_env --results-dir=results with T=$T budget=$BUDGET save_every=$SAVE_EVERY q=$Q mode=$MODE node_train=$NODE_TRAIN node_test=$NODE_TEST m=$M propagate_p=$PROPAGATE_P p=$P method=$METHOD reward_type=$REWARD_TYPE greedy_sample_size=$GREEDY_SAMPLE_SIZE graph_type=$GRAPH_TYPE is_real_graph=$IS_REAL_GRAPH sample_nodes_ratio=$SAMPLE_NODES_RATIO cascade=$CASCADE real_graph_name=$REAL_GRAPH_NAME lr=$LR epsilon_decay_steps=$EPSILON_DECAY_STEPS num_simul_train=$NUM_SIMUL_TRAIN checkpoint_path=$CHECK_POINT_PATH load_step=$LOAD_STEP
